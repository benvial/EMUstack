...
superstrate = objects.ThinFilm(
	period, height_nm="semi_inf", material=materials.Air, world_1d=True
)
...
def simulate_stack(Re):
	...
	for Im in Im_n:
		TF_1 = objects.ThinFilm(
			period, height_nm=10, material=materials.Material(Re + Im * 1j)
		)
...
ax1.set_xticks(np.linspace(n_min, (num_n_re - 1), n_max))
ax1.set_yticks(np.linspace(n_min, (num_n_im - 1), n_max))
ax1.set_xticklabels(
	[str(i) for i in np.linspace(n_min, n_max, n_max - n_min + 1)]
)
ax1.set_yticklabels(
	[str(i) for i in np.linspace(n_max, n_min, n_max - n_min + 1)]
)
