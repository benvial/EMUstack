...
def save_reference_data(casefile_name, stack_list):
	...
	np.savez_compressed(f"ref/{casefile_name}.npz", stack_list=ref_stack_list)

	cp_cmd = f"cp *.txt ref/{casefile_name}/"
	ossys(cp_cmd)
...
def results_match_reference(case, rtol, atol, result_files):
	for filename in result_files:
		reference = np.loadtxt(
			os.path.join(ref_path, f"case_{case}/" + filename)
		)
...
def check_results_simu_npz(case, rtol, atol, stack_list):
	ref = np.load(
		os.path.join(ref_path, f"case_{case}.npz"),
		allow_pickle=True,
		encoding="latin1",
	)
	...
	for stack, rstack in zip(stack_list, ref["stack_list"]):
		...
		lbl_s = f"wl = {stack.layers[0].light.wl_nm:f}, "
