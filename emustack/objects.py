...
def print_aknowledgements():
	"""Print acknowledgement message to screen.
...
class NanoStruct:
	...
	def __init__(
		...
		if ff == 0:
			...
			if diameter2 != 0:
				self.diameter2 = (
					2
					* ((ff * (period) ** 2) / np.pi - ((diameter1 / 2) ** 2))
					** 0.5
				)
			else:
	...
	def make_mesh(self):
		...
		if self.periodicity == "2D_array":
			if self.inc_shape in ["circle", "ellipse", "square"]:
				if self.diameter10 > 0:
					...
					if self.is_hex is False:
						msh_name = gen_msh_name(
							self.period, self.period_y, self.diameter1
						)
					elif self.is_hex is True:
						msh_name = "hex_" + gen_msh_name(
							self.period, self.period_y, self.diameter1
						)
				else:
					raise ValueError(
				...
				if self.ellipticity != 0:
					msh_name = (
						msh_name + f"_e_{dec_float_str(self.ellipticity)}"
					)
				if self.inc_shape == "square":
					msh_name = msh_name + "_sq"
				if self.posx != 0:
					msh_name = msh_name + f"x{dec_float_str(self.posx)}"
				if self.posy != 0:
					msh_name = msh_name + f"y{dec_float_str(self.posy)}"
				...
				if self.ff_rand is True:
					...
					np.sqrt((self.ff * (unit_period) ** 2) / np.pi)
					test_ff = 0
				...
				if (
					not os.path.exists(msh_location + msh_name + ".mail")
					or self.force_mesh is True
				):
					if self.is_hex is False:
						geo_tmp = open(
							template_location + f"{supercell}_msh_template.geo"
						).read()
					else:
						geo_tmp = open(
					...
					geo = geo_tmp.replace("ff = 0;", f"ff = {self.ff:f};")
					geo = geo.replace(
						"d_in_nm = 0;", f"d_in_nm = {self.period:f};"
					)
					geo = geo.replace(
						"dy_in_nm = 0;", f"dy_in_nm = {self.period_y:f};"
					)
					geo = geo.replace("a1 = 0;", f"a1 = {self.diameter1:f};")
					geo = geo.replace(
						"ellipticity = 0;",
						f"ellipticity = {self.ellipticity:f};",
					)
					if self.inc_shape == "square":
						geo = geo.replace("square = 0;", "square = 1;")
					geo = geo.replace("lc = 0;", f"lc = {self.lc:f};")
					geo = geo.replace("lc2 = lc/1;", f"lc2 = lc/{self.lc2:f};")
					geo = geo.replace("lc3 = lc/1;", f"lc3 = lc/{self.lc3:f};")
					if self.posx != 0:
						# appropriate for old definition of fraction of
						...
						geo = geo.replace(
							...
							% (
								...
								/ self.period
								* (
									self.period / (2 * np.sqrt(supercell))
									- self.diameter1 / 2.0
								)
							),
						)
					...
					if supercell > 1:
						geo = geo.replace(
							"a2 = 0;", f"a2 = {self.diameter2:f};"
						)
						geo = geo.replace(
							"lc4 = lc/1;", f"lc4 = lc/{self.lc4:f};"
						)
					if supercell > 2:
						geo = geo.replace(
							"a3 = 0;", f"a3 = {self.diameter3:f};"
						)
						geo = geo.replace(
							"lc5 = lc/1;", f"lc5 = lc/{self.lc5:f};"
						)
					if supercell > 3:
						geo = geo.replace(
							"a4 = 0;", f"a4 = {self.diameter4:f};"
						)
						geo = geo.replace(
							"lc6 = lc/1;", f"lc6 = lc/{self.lc6:f};"
						)
					if supercell > 4:
						geo = geo.replace(
							"a5 = 0;", f"a5 = {self.diameter5:f};"
						)
						geo = geo.replace(
							"a6 = 0;", f"a6 = {self.diameter6:f};"
						)
						geo = geo.replace(
							"a7 = 0;", f"a7 = {self.diameter7:f};"
						)
						geo = geo.replace(
							"a8 = 0;", f"a8 = {self.diameter8:f};"
						)
						geo = geo.replace(
							"a9 = 0;", f"a9 = {self.diameter9:f};"
						)
					if supercell > 9:
						geo = geo.replace(
							"a10 = 0;", f"a10 = {self.diameter10:f};"
						)
						geo = geo.replace(
							"a11 = 0;", f"a11 = {self.diameter11:f};"
						)
						geo = geo.replace(
							"a12 = 0;", f"a12 = {self.diameter12:f};"
						)
						geo = geo.replace(
							"a13 = 0;", f"a13 = {self.diameter13:f};"
						)
						geo = geo.replace(
							"a14 = 0;", f"a14 = {self.diameter14:f};"
						)
						geo = geo.replace(
							"a15 = 0;", f"a15 = {self.diameter15:f};"
						)
						geo = geo.replace(
							"a16 = 0;", f"a16 = {self.diameter16:f};"
						)

			elif self.inc_shape == "SRR":
				msh_name = "SRR_" + gen_msh_name(
					self.period,
					self.period_y,
					self.len_vertical,
					self.len_horizontal,
				...
				if (
					...
					geo_tmp = open(
						template_location + "SRR_msh_template.geo"
					).read()
					geo = geo_tmp.replace("ff = 0;", f"ff = {self.ff:f};")
					geo = geo.replace(
						"d_in_nm  = 0;", f"d_in_nm  = {self.period:f};"
					...
					geo = geo.replace(
						"lvert_nm = 0;", f"lvert_nm = {self.len_vertical:f};"
					)
					geo = geo.replace(
						"lhori_nm = 0;", f"lhori_nm = {self.len_horizontal:f};"
					)
					geo = geo.replace(
						"width_nm = 0;", f"width_nm = {self.diameter1:f};"
					)
					geo = geo.replace("lc = 0;", f"lc = {self.lc:f};")
					geo = geo.replace("lc2 = lc/1;", f"lc2 = lc/{self.lc2:f};")
					geo = geo.replace("lc3 = lc/1;", f"lc3 = lc/{self.lc3:f};")
				...
				msh_name = "ring_" + gen_msh_name(
					self.period, self.period_y, self.diameter1, self.diameter2
				)
				if (
					not os.path.exists(msh_location + msh_name + ".mail")
					or self.force_mesh is True
					...
					geo = geo.replace("a1 = 0;", f"a1 = {self.diameter1:f};")
					geo = geo.replace("a2 = 0;", f"a2 = {self.diameter2:f};")
					geo = geo.replace("lc = 0;", f"lc = {self.lc:f};")
					geo = geo.replace("lc2 = lc/1;", f"lc2 = lc/{self.lc2:f};")
					geo = geo.replace("lc3 = lc/1;", f"lc3 = lc/{self.lc3:f};")
					geo = geo.replace(
						"xshift_nm = 0;", f"xshift_nm = {self.xshift:f};"
					)

			elif self.inc_shape == "dimer":
				msh_name = "dimer_" + gen_msh_name(
					self.period,
					self.period_y,
					self.diameter1,
					self.diameter2,
				...
				if (
					...
					geo = geo.replace("a1 = 0;", f"a1 = {self.diameter1:f};")
					geo = geo.replace("a2 = 0;", f"a2 = {self.diameter2:f};")
					geo = geo.replace("gap = 0;", f"gap = {self.gap:f};")
					geo = geo.replace("lc = 0;", f"lc = {self.lc:f};")
					geo = geo.replace("lc2 = lc/1;", f"lc2 = lc/{self.lc2:f};")
					geo = geo.replace("lc3 = lc/1;", f"lc3 = lc/{self.lc3:f};")

			elif self.inc_shape == "square_dimer":
				msh_name = "square_dimer_" + gen_msh_name(
					self.period,
					self.period_y,
					self.diameter1,
					self.diameter2,
				...
				if (
					...
					geo = geo.replace("gap = 0;", f"gap = {self.gap:f};")
					geo = geo.replace(
						"smooth = 0;", f"smooth = {self.smooth:f};"
					)
					geo = geo.replace("lc = 0;", f"lc = {self.lc:f};")
					geo = geo.replace("lc2 = lc/1;", f"lc2 = lc/{self.lc2:f};")
					geo = geo.replace("lc3 = lc/1;", f"lc3 = lc/{self.lc3:f};")
				...
				msh_name = "square_shell_dimer_" + gen_msh_name(
					self.period,
					self.period_y,
					self.diameter1,
					self.diameter2,
				...
				if (
					...
					geo_tmp = open(
						template_location
						+ "square_shell_dimer1_msh_template.geo"
					).read()
					geo = geo_tmp.replace("ff = 0;", f"ff = {self.ff:f};")
					geo = geo.replace(
						"d_in_nm = 0;", f"d_in_nm  = {self.period:f};"
					...
					geo = geo.replace("a1 = 0;", f"a1 = {self.diameter1:f};")
					geo = geo.replace("a2 = 0;", f"a2 = {self.diameter2:f};")
					geo = geo.replace("b1 = 0;", f"b1 = {self.diameter3:f};")
					geo = geo.replace("b2 = 0;", f"b2 = {self.diameter4:f};")
					geo = geo.replace("gap = 0;", f"gap = {self.gap:f};")
					geo = geo.replace(
						"smooth = 0;", f"smooth = {self.smooth:f};"
					)
					geo = geo.replace("t = 0;", f"t = {self.t:f};")
					geo = geo.replace("lc = 0;", f"lc = {self.lc:f};")
					geo = geo.replace("lc2 = lc/1;", f"lc2 = lc/{self.lc2:f};")
					geo = geo.replace("lc3 = lc/1;", f"lc3 = lc/{self.lc3:f};")
			elif self.inc_shape == "rectangle":
				msh_name = "rectangle_" + gen_msh_name(
					self.period,
					self.period_y,
					self.diameter1,
					self.diameter2,
				...
				msh_name = "rect_shell_" + gen_msh_name(
					self.period,
					self.period_y,
					self.diameter1,
					self.diameter2,
				...
				if (
					...
					geo = geo.replace("a1 = 0;", f"a1 = {self.diameter1:f};")
					geo = geo.replace("b1 = 0;", f"b1 = {self.diameter2:f};")
					geo = geo.replace(
						"smooth = 0;", f"smooth = {self.smooth:f};"
					)
					geo = geo.replace("t = 0;", f"t = {self.t:f};")
				...
				msh_name = "cross_shell_" + gen_msh_name(
					self.period,
					self.period_y,
					self.diameter1,
					self.diameter2,
				...
				if (
					...
					geo_tmp = open(
						template_location + "L_msh_template.geo"
					).read()
					geo = geo_tmp.replace("ff = 0;", f"ff = {self.ff:f};")
					geo = geo.replace(
						"d_in_nm = 0;", f"d_in_nm  = {self.period:f};"
					...
					geo = geo.replace(
						"L_nm = 0;", f"L_nm = {self.diameter1:f};"
					)
					geo = geo.replace(
						"W_nm = 0;", f"W_nm = {self.diameter2:f};"
					)
					geo = geo.replace("r = 0;", f"r = {self.smooth:f};")
					geo = geo.replace("lc = 0;", f"lc = {self.lc:f};")
					geo = geo.replace("lc2 = lc/1;", f"lc2 = lc/{self.lc2:f};")
			elif self.inc_shape == "strip_circle":
				msh_name = "strip_circle_" + gen_msh_name(
					self.period,
					self.period_y,
					self.diameter1,
					self.diameter2,
				...
				if (
					...
					geo = geo.replace("a1 = 0;", f"a1 = {self.diameter1:f};")
					geo = geo.replace(
						"strip = 0;", f"strip = {self.diameter2:f};"
					)
					geo = geo.replace("lc = 0;", f"lc = {self.lc:f};")
					geo = geo.replace("lc2 = lc/1;", f"lc2 = lc/{self.lc2:f};")
					geo = geo.replace("lc3 = lc/1;", f"lc3 = lc/{self.lc3:f};")
				...
				msh_name = "strip_square_" + gen_msh_name(
					self.period,
					self.period_y,
					self.diameter1,
					self.diameter2,
				...
				msh_name = "double_strip_circle_" + gen_msh_name(
					self.period,
					self.period_y,
					self.diameter1,
					self.diameter2,
				...
				if (
					...
					geo_tmp = open(
						msh_location + "1_2strip_msh_template.geo"
					).read()
					geo = geo_tmp.replace("ff = 0;", f"ff = {self.ff:f};")
					geo = geo.replace(
						"d_in_nm = 0;", f"d_in_nm  = {self.period:f};"
					...
					geo = geo.replace(
						"strip2 = 0;", f"strip2 = {self.diameter3:f};"
					)
					geo = geo.replace("lc = 0;", f"lc = {self.lc:f};")
					geo = geo.replace("lc2 = lc/1;", f"lc2 = lc/{self.lc2:f};")
					geo = geo.replace("lc3 = lc/1;", f"lc3 = lc/{self.lc3:f};")
				...
				msh_name = "double_strip_square_" + gen_msh_name(
					self.period,
					self.period_y,
					self.diameter1,
					self.diameter2,
				...
				if self.geo_params is None:
					raise TypeError(
						'Must provide geo_params dictionary if inc_shape="custom".'  # noqa: E501
					)
				...
				if self.geo_file[-13:] != "_template.geo":
					raise ValueError(
						"Provided .geo filename must respect the "
						"'*_template.geo' naming convention."
					)
				...
				msh_name = (
					root_msh_name
					+ f"{dec_float_str(self.period)}"
					+ f"_{dec_float_str(self.period_y)}_"
				)

				# add custom parameter values to mesh name
				for key, value in self.geo_params.items():
					msh_name = msh_name + "_" + key + f"{dec_float_str(value)}"
				...
				if (
					...
					geo_tmp = open(msh_location + self.geo_file).read()
					geo = geo_tmp.replace(
						"d_in_nm = 0;", f"d_in_nm  = {self.period:f};"
					)
					geo = geo.replace(
					...
					for key, value in self.geo_params.items():
						try:
							if key[0:2] == "lc":
								str_in = key + " = lc/1;"
								str_out = key + f" = lc/{value:f};"
							else:
								str_in = key + " = 0;"
								str_out = key + f" = {value:f};"
				...
				raise NotImplementedError(
					f"Selected inc_shape '{self.inc_shape}' "
			...
			type_el = np.zeros(nel + 1, dtype=int)
			ls_x = np.zeros(npt + 1)
			...
			if self.diameter6 > 0:
				...
				if self.edge_spacing is True:
					i_d = (
						2.0
						* (0.5 - rad_1 - rad_2 - rad_3 - rad_4 - rad_5 - rad_6)
						/ 6.0
					)
					for i_el in el_list:
						x_1 = ls_x[2 * i_el - 1]
						...
						if (
							x_1 <= 0.5 + rad_1
							and 0.5 - rad_1 - dx <= x_1
							and x_2 <= 0.5 + rad_1
							and 0.5 - rad_1 - dx <= x_2
						):
							type_el[i_el] = 2
						elif (
							x_1 <= 0.5 - i_d - rad_1
							and 0.5 - i_d - rad_1 - 2.0 * rad_2 <= x_1
							and x_2 <= 0.5 - i_d - rad_1
							and 0.5 - i_d - rad_1 - 2.0 * rad_2 <= x_2
						):
							type_el[i_el] = 3
						elif (
							x_1 <= 0.5 + i_d + 2.0 * rad_3 + rad_1
							and 0.5 + i_d + rad_1 <= x_1
							and x_2 <= 0.5 + i_d + 2.0 * rad_3 + rad_1
							and 0.5 + i_d + rad_1 <= x_2
						):
							type_el[i_el] = 4
						elif (
							x_1 <= 0.5 - 2.0 * i_d - 2.0 * rad_2 - rad_1
							and 0.5
							- 2.0 * i_d
							- 2.0 * rad_2
							- 2.0 * rad_4
							- rad_1
							<= x_1
							and x_2 <= 0.5 - 2.0 * i_d - 2.0 * rad_2 - rad_1
							and 0.5
							- 2.0 * i_d
							- 2.0 * rad_2
							- 2.0 * rad_4
							...
							0.5 + 2.0 * i_d + 2.0 * rad_3 + rad_1 <= x_1
							and x_1
							<= 0.5
							+ 2.0 * i_d
							+ 2.0 * rad_3
							+ 2.0 * rad_5
							+ rad_1
							and 0.5 + 2.0 * i_d + 2.0 * rad_3 + rad_1 <= x_2
							and x_2
							<= 0.5
							+ 2.0 * i_d
							+ 2.0 * rad_3
					...
					for i_el in el_list:
						...
						if (
							...
							x_1 <= 0.5 + rad_1 + rad_2
							and 0.5 + rad_1 <= x_1
							and x_2 <= 0.5 + rad_1 + rad_2
							and 0.5 + rad_1 <= x_2
						):
							type_el[i_el] = 3
						elif (
							x_1 <= 0.5 - rad_1
							and 0.5 - rad_1 - rad_2 <= x_1
							and x_2 <= 0.5 - rad_1
							and 0.5 - rad_1 - rad_2 <= x_2
						):
							type_el[i_el] = 3
						elif (
							x_1 <= 0.5 + rad_1 + rad_2 + rad_3
							and 0.5 + rad_1 + rad_2 <= x_1
							and x_2 <= 0.5 + rad_1 + rad_2 + rad_3
							and 0.5 + rad_1 + rad_2 <= x_2
						):
							type_el[i_el] = 4
						elif (
							x_1 <= 0.5 - rad_1 - rad_2
							and 0.5 - rad_1 - rad_2 - rad_3 <= x_1
							and x_2 <= 0.5 - rad_1 - rad_2
							and 0.5 - rad_1 - rad_2 - rad_3 <= x_2
						):
							type_el[i_el] = 4
						elif (
							x_1 <= 0.5 + rad_1 + rad_2 + rad_3 + rad_4
							and 0.5 + rad_1 + rad_2 + rad_3 <= x_1
							and x_2 <= 0.5 + rad_1 + rad_2 + rad_3 + rad_4
							and 0.5 + rad_1 + rad_2 + rad_3 <= x_2
						):
							type_el[i_el] = 5
						elif (
							x_1 <= 0.5 - rad_1 - rad_2 - rad_3
							and 0.5 - rad_1 - rad_2 - rad_3 - rad_4 <= x_1
							and x_2 <= 0.5 - rad_1 - rad_2 - rad_3
							and 0.5 - rad_1 - rad_2 - rad_3 - rad_4 <= x_2
						):
							type_el[i_el] = 5
						elif (
							x_1 <= 0.5 + rad_1 + rad_2 + rad_3 + rad_4 + rad_5
							and 0.5 + rad_1 + rad_2 + rad_3 + rad_4 <= x_1
							and x_2
							<= 0.5 + rad_1 + rad_2 + rad_3 + rad_4 + rad_5
							and 0.5 + rad_1 + rad_2 + rad_3 + rad_4 <= x_2
						):
							type_el[i_el] = 6
						elif (
							x_1 <= 0.5 - rad_1 - rad_2 - rad_3 - rad_4
							and 0.5 - rad_1 - rad_2 - rad_3 - rad_4 - rad_5
							<= x_1
							and x_2 <= 0.5 - rad_1 - rad_2 - rad_3 - rad_4
							and 0.5 - rad_1 - rad_2 - rad_3 - rad_4 - rad_5
							<= x_2
					...
					for i_el in el_list:
						...
						if (
							...
							0.5 - i_d - rad_2 <= x_1
							and x_1 <= 0.5 - i_d + rad_2
							and 0.5 - i_d - rad_2 <= x_2
							and x_2 <= 0.5 - i_d + rad_2
						):
							type_el[i_el] = 3
						elif (
							0.5 + i_d - rad_3 <= x_1
							and x_1 <= 0.5 + i_d + rad_3
							and 0.5 + i_d - rad_3 <= x_2
							and x_2 <= 0.5 + i_d + rad_3
						):
							type_el[i_el] = 4
						elif (
							0.5 - 2.0 * i_d - rad_4 <= x_1
							and x_1 <= 0.5 - 2.0 * i_d + rad_4
							and 0.5 - 2.0 * i_d - rad_4 <= x_2
							and x_2 <= 0.5 - 2.0 * i_d + rad_4
						):
							type_el[i_el] = 5
						elif (
							0.5 + 2.0 * i_d - rad_5 <= x_1
							and x_1 <= 0.5 + 2.0 * i_d + rad_5
							and 0.5 + 2.0 * i_d - rad_5 <= x_2
							and x_2 <= 0.5 + 2.0 * i_d + rad_5
						):
							type_el[i_el] = 6
				...
				if self.edge_spacing is True:
					...
					for i_el in el_list:
						...
						if (
							...
							0.5 + i_d + rad_1 <= x_1
							and x_1 <= 0.5 + i_d + rad_1 + 2.0 * rad_3
							and 0.5 + i_d + rad_1 <= x_2
							and x_2 <= 0.5 + i_d + rad_1 + 2.0 * rad_3
						):
							type_el[i_el] = 4
						elif (
							x_1 >= 0.5 + 2.0 * i_d + rad_1 + 2.0 * rad_3
							and x_2 >= 0.5 + 2.0 * i_d + rad_1 + 2.0 * rad_3
						):
							type_el[i_el] = 4
						elif (
							x_1 <= 0.5 - 2.0 * i_d - rad_1 - 2.0 * rad_2
				...
				msh_name = gen_msh_name(
					self.period, self.diameter1, self.diameter2, self.diameter3
				)
				# End-points of the elements
				rad_1 = self.diameter1 / (2.0 * self.period)
				rad_2 = self.diameter2 / (2.0 * self.period)
				...
				if self.edge_spacing is True:
					i_d = (
						1.0 - self.diameter1 - self.diameter2 - self.diameter3
					) / 3.0
					for i_el in el_list:
						x_1 = ls_x[2 * i_el - 1]
						x_2 = ls_x[2 * i_el + 1]
						...
						# inclusion 2
							...
							0.5 - i_d - 2.0 * rad_2 - rad_1 <= x_1
							and x_1 <= 0.5 - i_d - rad_1
							and 0.5 - i_d - 2.0 * rad_2 - rad_1 <= x_2
							and x_2 <= 0.5 - i_d - rad_1
						):
							type_el[i_el] = 3
						# inclusion 3
					...
					for i_el in el_list:
						...
						if (
							...
							x_1 <= 0.5 - rad_1
							and 0.5 - rad_1 - rad_2 - dx <= x_1
							and x_2 <= 0.5 - rad_1
							and 0.5 - rad_1 - rad_2 - dx <= x_2
						):
							type_el[i_el] = 3
						elif (
							x_1 <= 0.5 + rad_1 + rad_2 + rad_3
							...
							x_1 <= 0.5 - rad_1 - rad_2
							and 0.5 - rad_1 - rad_2 - rad_3 - dx <= x_1
							and x_2 <= 0.5 - rad_1 - rad_2
							and 0.5 - rad_1 - rad_2 - rad_3 - dx <= x_2
						):
							type_el[i_el] = 4
						else:
							type_el[i_el] = 1
				...
				msh_name = gen_msh_name(
					self.period, self.diameter1, self.diameter2
				)
				# End-points of the elements
				rad_1 = self.diameter1 / (2.0 * self.period)
				rad_2 = self.diameter2 / (2.0 * self.period)
				if self.split_touching_incs is True:
					...
					for i_el in el_list:
						...
						if (
							...
							x_1 <= 0.5 - rad_1 - small_space
							and x_2 <= 0.5 - rad_1 - small_space
						):
							type_el[i_el] = 3
						elif (
							x_1 >= 0.5 + large_d + rad_1
							and x_2 >= 0.5 + large_d + rad_1
						):
							type_el[i_el] = 3
						else:
							type_el[i_el] = 1
			elif self.diameter1 > 0:
				msh_name = gen_msh_name(self.period, self.diameter1)
				# End-points of the elements
				rad_1 = self.diameter1 / (2.0 * self.period)
				for i_el in el_list:
					x_1 = ls_x[2 * i_el - 1]
...
class Light:
	...
	def __init__(
		...
		if np.imag(wl_nm) != 0:
			...
			print(
				"Warning: using a complex wavelength. "
				"EMUstack can only handle these for uniform films "
				"using 0 pw_orders."
			)
		else:
		...
		if None is theta and None is k_parallel:
			raise ValueError(
				"Specify incident angle either by k_parallel "
				"OR by theta, phi and n_inc."
			)
		...
		if None is theta:
			...
			if np.abs(self.k_pll[0]) == 0 or np.abs(self.k_pll[1]) == 0:
				print(
					"Warning: a component of k_parallel is exactly zero, "
					"this can lead to degeneracies and errors."
				)
		else:
			# Check for inconsistent input
			if None is not k_parallel or phi is None:
				raise ValueError(
					"Specify incident angle either by k_parallel "
					"OR by theta, phi and n_inc."
				)
			...
			self.k_pll = (
				k
				* np.sin(theta)
				* np.array([np.cos(phi), np.sin(phi)], dtype="float64")
			)
...
def dec_float_str(dec_float):
	"""Convert float with decimal point into string."""
	string = str(dec_float)
	fmt_string = string.replace(".", "_")
	return fmt_string


def gen_msh_name(*args):
	"""Generate a filename-safe string from a sequence of floats.

	Replace '.' with '_' in the string representation of each float, and
	join the strings together with '_' in between.

	Example:
	        gen_msh_name(3.14, 2.718, 1.618) -> "3_14_2_718_1_618"
	"""
	return "_".join(dec_float_str(x) for x in args)
