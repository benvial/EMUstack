...
from .mode_calcs import Anallo, r_t_mat
...
class Stack:
	...
	def heights_nm(self):
		...
		if self._heights_nm is not None:
			...
			return [
				float(lay.structure.height_nm) for lay in self.layers[1:-1]
			]
	...
	def calc_scat(
		...
		"""
		# Can check this against lines ~ 127 in J_overlap.f E components
		# are TE, have diffraction orders along beta, which is along y.
		...
		for i in range(1, len(self.layers) - 1):
			...
			if self.shears is None:
				...
				rnet = (
					r21_list[i] + t12_list[i] @ Q_inv @ rnet @ Q @ inverted_t21
				)
		...
		if self.shears is None:
			...
			tnet = tnet @ Q @ inverted_t21
			rnet = (
				r21_list[-1] + t12_list[-1] @ Q_inv @ rnet @ Q @ inverted_t21
			)
		inv_t21_list.append(inverted_t21)
		tnet_list.append(tnet)
		rnet_list.append(rnet)
		...
		if calc_fluxes is True:
			...
			# It is a diagonal matrix with 1 for propagating,
			# i for evanescent TE & -i for evanescent TM plane wave orders.
			...
			flux_TM = (
				np.linalg.norm(d_minus[neq_PW : neq_PW + num_prop_in]) ** 2
			)
			...
			flux_TM = (
				np.linalg.norm(d_plus[neq_PW : neq_PW + num_prop_in]) ** 2
			)
			...
			if isinstance(self.layers[0], Anallo):
				...
				if num_prop_out != 0:
					...
					flux_TM = (
						np.linalg.norm(
							f2_minus[neq_PW : neq_PW + num_prop_out]
						)
						** 2
					)
					down_fluxes.append(flux_TE + flux_TM)
					...
					print(
						"Warning: there are no propagating modes "
						"in the semi-infinite substrate therefore "
						"CANNOT CALCULATE ENERGY FLUXES ANYWHERE."
					)
				...
				if num_prop_out != 0:
					...
					a_layer = abs(
						down_fluxes[0] - down_fluxes[-1] - up_flux[0]
					)
					self.a_list.append(a_layer)
					...
					print(
						"Warning: there are no propagating modes "
						"in the semi-infinite superstrate therefore "
						"CANNOT CALCULATE ENERGY FLUXES ANYWHERE."
					)
	...
	def _check_periods_are_consistent(self):
		...
		for lay in self.layers:
			assert (
				lay.structure.period == self.period
			), "All layers in a multilayer stack must have the same period."
			assert (
				lay.structure.period_y == self.period_y
