...
class Modes(object):
    ...
    def air_ref(self):
        ...
        return self.light._air_ref(
            self.structure.period, self.structure.period_y, self.structure.world_1d
        )
    ...
    def specular_incidence(self, pol="TE"):
        ...
        if "TE" == pol:
            ...
            raise NotImplementedError(
                "Must select from the currently implemented polarisations; TE, TM, R Circ, L Circ."
            )
...
class Simmo(Modes):
    ...
    def calc_modes(self, num_BMs=None):
        ...
        if self.structure.periodicity == "1D_array":
            ...
            raise ValueError(
                "NanoStruct layer must have periodicity of either '1D_array' or '2D_array'."
            )
        ...
        if self.structure.periodicity == "1D_array":
            ...
            try:
                ...
                print(
                    "\n\n1D FEM routine calc_modes_1d", "interrupted by keyboard.\n\n"
                )
            ...
            try:
                ...
                print(
                    "\n\n2D FEM routine calc_modes_2d", "interrupted by keyboard.\n\n"
                )
...
def r_t_mat(lay1, lay2):
    ...
    if isinstance(lay1, Anallo) and isinstance(lay2, Anallo):
        ...
        raise NotImplementedError(
            "Sorry! For, now you can put an extremely thin film between your NanoStructs"
        )
...
def r_t_mat_anallo(an1, an2):
    ...
    if len(an1.k_z) != len(an2.k_z):
        raise ValueError(
            "Need the same number of plane waves in Anallos %(an1)s and %(an2)s"
            % {"an1": an1, "an2": an2}
        )
...
def r_t_mat_tf_ns(an1, sim2):
    ...
    if np.shape(Z1_sqrt_inv)[1] != np.shape(sim2.J)[0]:
        raise ValueError(
            "Scattering matrices of layers are not consistent, some layers are 1D and others 2D. Check that world_1d status."
        )
