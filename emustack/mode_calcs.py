...
class Modes:
	...
	def k_pll_norm(self):
		"""Parallel k-vector norm.

		Return the parallel component of the wave vector of
		the incident light, scaled by the period of the structure.
		"""
	...
	def air_ref(self):
		...
		return self.light._air_ref(
			self.structure.period,
	...
	def shear_transform(self, coords):
		...
		if np.shape(coords) == (1,):
			...
			return np.array(
				np.diag(np.exp(1j * (alphas * coords[0] + betas * coords[1])))
			)
...
class Anallo(Modes):
	...
	def calc_kz(self):
		...
		if np.shape(np.nonzero(k_z_unsrt))[1] != np.shape(k_z_unsrt)[0]:
			print(
				"Warning: selected [k_x, k_y] hits a Wood Anomaly!"
		...
		if self.structure.world_1d is True:
			...
			self.specular_order = np.nonzero((pxs[s] == 0) * (pys[s] == 0))[0][
				0
			]
	...
	def specular_incidence(self, pol="TE"):
		"""Return a vector of plane wave amplitudes.

		This corresponds to specular incidence in the specified polarisation.
		...
		if "TE" == pol:
			...
			inc_amp[spec_TE] = 1 / np.sqrt(2.0)
			inc_amp[spec_TM] = 1 / np.sqrt(2.0)
		elif "R Circ" == pol:
			inc_amp[spec_TE] = 1 / np.sqrt(2.0)
			inc_amp[spec_TM] = +1j / np.sqrt(2.0)
		elif "L Circ" == pol:
			inc_amp[spec_TE] = 1 / np.sqrt(2.0)
			inc_amp[spec_TM] = -1j / np.sqrt(2.0)
		else:
...
class Simmo(Modes):
	...
	def calc_modes(self, num_BMs=None):
		...
		if self.structure.periodicity == "1D_array":
			...
			raise ValueError(
				"NanoStruct layer must have periodicity of either '1D_array' "
				"or '2D_array'."
			)

		num_pw_per_pol = pxs.size
		if num_BMs is None:
			self.num_BMs = num_pw_per_pol * 2 + 20
		else:
		...
		if self.structure.hyperbolic is True:
		...
		if self.structure.periodicity == "1D_array":
			...
			try:
				resm = EMUstack.calc_modes_1d(
					...
					self.num_BMs,
					self.max_order_PWs,
					self.structure.nb_typ_el,
					self.structure.n_msh_pts,
					self.structure.n_msh_el,
					self.structure.table_nod,
					self.structure.type_el,
					self.structure.x_arr,
					itermax,
					FEM_debug,
					self.structure.mesh_file,
					self.n_effs,
					self.k_pll_norm()[0],
					self.k_pll_norm()[1],
					shift,
					self.structure.plotting_fields,
					self.structure.plot_real,
					self.structure.plot_imag,
					self.structure.plot_abs,
					num_pw_per_pol,
					num_pw_per_pol_2d,
					world_1d,
				)
...
def r_t_mat(lay1, lay2):
	...
	if isinstance(lay1, Anallo) and isinstance(lay2, Anallo):
		...
		raise NotImplementedError(
			"Sorry! For, now you can put an extremely thin film "
			"between your NanoStructs"
		)
...
def r_t_mat_anallo(an1, an2):
	"""Return R12, T12, R21, T21 at an interface between thin films.
	...
	if len(an1.k_z) != len(an2.k_z):
		raise ValueError(
			f"Need the same number of plane waves in Anallos {an1} and {an2}"
		)
	...
	T12 = np.array(np.diag(2.0 * np.sqrt(Z2) * np.sqrt(Z1) / (Z2 + Z1)))
	R21 = -R12
	T21 = T12
...
def r_t_mat_tf_ns(an1, sim2):
	"""Return R12, T12, R21, T21 at an1-sim2 interface.
	...
	if np.shape(Z1_sqrt_inv)[1] != np.shape(sim2.J)[0]:
		raise ValueError(
			"Scattering matrices of layers are not consistent, "
