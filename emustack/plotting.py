...
def max_n(stacks_list):
	...
	for s in stacks_list:
		for lindex in s.layers:
			if isinstance(lindex, objects.Anallo):
				ns.append(lindex.n())
			if isinstance(lindex, objects.Simmo):
				wl = lindex.light.wl_nm
				ns.append(lindex.structure.background.n(wl))
				ns.append(lindex.structure.inclusion_a.n(wl))
				ns.append(lindex.structure.inclusion_b.n(wl))
...
def gen_params_string(stack, layer=1):
	...
	if isinstance(param_layer, objects.NanoStruct):
		params_2_print = f"ff = {param_layer.ff:5.3f}, "
		params_2_print += "d = %(period)d, a1 = %(diameter)d, " % {
			"period": param_layer.period,
			"diameter": param_layer.diameter1,
		}
		if param_layer.diameter2 != 0:
			params_2_print += "a2 = %(rad)d " % {
				"rad": param_layer.diameter2,
			}
		if param_layer.periodicity == "2D_array":
			if param_layer.diameter3 != 0:
				params_2_print += "a3 = %(rad)d " % {
					"rad": param_layer.diameter3,
				}
			if param_layer.diameter4 != 0:
				params_2_print += "a4 = %(rad)d " % {
					"rad": param_layer.diameter4,
				}
			if param_layer.diameter5 != 0:
				params_2_print += "\na5 = %(rad)d " % {
					"rad": param_layer.diameter5,
				}
			if param_layer.diameter6 != 0:
				params_2_print += "a6 = %(rad)d " % {
					"rad": param_layer.diameter6,
				}
			if param_layer.diameter7 != 0:
				params_2_print += "a7 = %(rad)d " % {
					"rad": param_layer.diameter7,
				}
			if param_layer.diameter8 != 0:
				params_2_print += "a8 = %(rad)d " % {
					"rad": param_layer.diameter8,
				}
			if param_layer.diameter9 != 0:
				params_2_print += "a9 = %(rad)d \n" % {
					"rad": param_layer.diameter9,
				}
			if param_layer.diameter10 != 0:
				params_2_print += "a10 = %(rad)d " % {
					"rad": param_layer.diameter10,
				}
			if param_layer.diameter11 != 0:
				params_2_print += "a11 = %(rad)d " % {
					"rad": param_layer.diameter11,
				}
			if param_layer.diameter12 != 0:
				params_2_print += "a12 = %(rad)d " % {
					"rad": param_layer.diameter12,
				}
			if param_layer.diameter13 != 0:
				params_2_print += "a13 = %(rad)d " % {
					"rad": param_layer.diameter13,
				}
			if param_layer.diameter14 != 0:
				params_2_print += "a14 = %(rad)d " % {
					"rad": param_layer.diameter14,
				}
			if param_layer.diameter15 != 0:
				params_2_print += "a15 = %(rad)d " % {
					"rad": param_layer.diameter15,
				}
			if param_layer.diameter16 != 0:
				params_2_print += "a16 = %(rad)d \n" % {
					"rad": param_layer.diameter16,
				}
			if param_layer.inc_shape == "square":
				params_2_print += "\nSquare ins "
			if param_layer.inc_shape == "ellipse":
				params_2_print += (
					f"\nEllipticity = {param_layer.ellipticity:5.3f} "
				)
...
def t_r_a_plots(
	...
	params_2_print += "\n" r"$h_t,...,h_b$ = "
	params_2_print += "".join(f"{num:4f}, " for num in height_list)

	if xvalues is None:
		if (
			stacks_list[0].layers[0].light.wl_nm
			!= stacks_list[-1].layers[0].light.wl_nm
		):
			xvalues = [s.layers[0].light.wl_nm for s in stacks_list]
			xlabel = r"$\lambda$ (nm)"
		elif set(stacks_list[0].layers[0].light.k_pll) != set(
			stacks_list[-1].layers[0].light.k_pll
		):
			xvalues = [
				np.sqrt(
					s.layers[0].light.k_pll[0] ** 2
					+ s.layers[0].light.k_pll[1] ** 2
				)
				for s in stacks_list
			]
	...
	if add_height != 0:
		add_name += "_" + zeros_int_str(add_height)
	...
	if ult_eta is True or J_sc is True:
		...
		for i in range(len(xvalues)):
			active_abs.append(
				float(a_list[active_layer_nu - 1 + i * layers_steps])
			)
		out = []

	if ult_eta is True:
		Efficiency = ult_efficiency(
			active_abs,
			xvalues,
		...
		params_2_print += rf"$\eta$ = {Efficiency * 100:6.2f}"
		params_2_print += " %"
	...
	if J_sc is True:
		...
		params_2_print += rf"$J_{{sc}}$ = {J:6.2f}"
		params_2_print += r" mA/cm$^2$"
		out.append(J)

	if save_txt is True or save_pdf is True:
		total_h = sum(
			stacks_list[0].heights_nm()
		)  # look at first wl result to find h.
	...
	if weight_spec is True:
		# Plot totals weighted by solar irradiance.
		i_data = np.loadtxt(f"{Irrad_spec_file}.txt")
...
def layers_plot(
	...
	"""Plot one type of spectrum across all layers.
	...
	for i in range(nu_layers):
		...
		if label_eV is True:
			...
			new_tick_labels = [f"{z:.1f}" for z in new_tick_values]
			ax2.set_xticks(new_tick_locations)
			ax2.set_xticklabels(new_tick_labels)
			if xvalues[0] != xvalues[-1]:
				ax2.set_xlim((xvalues[0], xvalues[-1]))
		if i == 0:
			if xlabel == r"$\lambda$ (nm)" and label_eV is True:
				ax2.set_xlabel("Energy (eV)")
		elif i == nu_layers - 1:
			ax1.set_xlabel(xlabel)
			ax1.set_ylabel("Total")
		if i != 0 and label_eV is True:
			ax2.set_xticklabels(())
		if i != nu_layers - 1:
			ax1.set_xticklabels(())
		if spectra_name == "Lay_Absorb":
			if i == 0:
				ax1.set_ylabel("Top Layer")
			if i == nu_layers - 2:
				ax1.set_ylabel("Bottom Layer")
			suptitle_w_params = (
				"Absorptance in each layer" + add_name + "\n" + params_2_print
			)
		...
		if save_txt is True:
			if i != nu_layers - 1:
				np.savetxt(
					"%(s)s_%(i)i_stack%(bon)s%(add)s.txt"
					% {
						"s": lay_spec_name,
						"i": i,
						"bon": stack_label,
				...
				np.savetxt(
					f"{lay_spec_name}_stack{stack_label}{add_name}.txt",
					av_array,
					fmt="%18.11f",
				)
		if save_pdf is True:
			plt.savefig(f"{spectra_name}_stack{stack_label}{add_name}")
	# fig.clf()
...
def total_tra_plot(
	...
	"""Plot total t, r, a spectra on one plot.
	...
	if save_pdf:
		plt.savefig(f"{plot_name}_stack{stack_label}{add_name}")
	# fig.clf()
...
def t_r_a_plots_subs(
	...
	if extinct is True:
		extinction_plot(
			t_tot, wavelengths, params_2_print, stack_label, add_name
		)
	if ult_eta is True or J_sc is True:
		del active_abs
	return
...
def total_tra_plot_subs(
	...
	"""Plot total t, r, a spectra with lines at first 6 Wood anomalies.
	...
	plt.suptitle(params_2_print, fontsize=title_font)
	# plt.suptitle(plot_name+add_name+'\n'+params_2_print)
	plt.savefig(f"{plot_name}_stack{stack_label}__{add_name}")
	# fig.clf()
...
def layers_print(
	spectra_name, spec_list, wavelengths, total_h, stack_label=1, add_name=""
):
	"""Save spectra to text files.

	Is called from t_r_a_write_files.
	"""
	nu_layers = int(len(spec_list) / len(wavelengths))
...
def extinction_plot(
	t_spec, wavelengths, params_2_print, stack_label, add_name
):
	"""Plot extinction ratio in transmission extinct = log_10(1/t)."""
	fig = plt.figure(
		num=None, figsize=(9, 12), dpi=80, facecolor="w", edgecolor="k"
	)
	...
	plt.savefig(f"{plot_name}_stack{stack_label}_{add_name}")
	# fig.clf()
...
def EOT_plot(
	...
	plot_name = "EOT"
	plt.suptitle(params_2_print, fontsize=title_font)
	plt.savefig(f"{plot_name}{add_name}")
	# fig.clf()

	if savetxt is True:
		np.savetxt(
			f"Trans_{plot_name}{add_name}.txt",
			list(zip(wavelengths, T_00)),
			fmt="%10.6f",
		)
		np.savetxt(
			f"Refl_{plot_name}{add_name}.txt",
			list(zip(wavelengths, R_00)),
			fmt="%10.6f",
		)
...
def J_short_circuit(
	active_abs, wavelengths, params_2_print="", stack_label="", add_name=""
):
	...
	np.savetxt(
		f"J_sc_stack{stack_label}{add_name}.txt",
		np.array([J]),
		fmt="%10.6f",
	)
	return J
...
def omega_plot(stacks_list, wavelengths, params_layer=1, stack_label=1):
	"""Plot the dispersion diagram of each layer in one plot.
	...
	for lindex in range(num_layers):
		ax1 = fig1.add_subplot(num_layers, 1, num_layers - lindex)
		ax2 = fig2.add_subplot(num_layers, 1, num_layers - lindex)
		ax3 = fig3.add_subplot(num_layers, 1, num_layers - lindex)
		ax4 = fig4.add_subplot(num_layers, 1, num_layers - lindex)
		for i in range(len(wavelengths)):
			k_zs = stacks_list[i].layers[lindex].k_z
			real_k_zs = []
			imag_k_zs = []
			for k in k_zs:
		...
		(
			ax1.set_ylabel(r"Real $k_z$ (d)"),
			ax2.set_ylabel(r"Imaginary $k_z$ (d)"),
		)
		(
			ax3.set_ylabel(r"Frequency ($\omega$d/2$\pi$c)"),
			ax4.set_ylabel(r"Frequency ($\omega$d/2$\pi$c)"),
		)
		if lindex == 0:
			ax1.set_ylabel("Bottom Layer"), ax2.set_ylabel("Bottom Layer")
			ax3.set_ylabel("Bottom Layer"), ax4.set_ylabel("Bottom Layer")
			(
				ax3.set_xlabel(r"Real $k_z$ (d)"),
				ax4.set_xlabel(r"Imaginary $k_z$ (d)"),
			)
			(
				ax1.set_xlabel("Wavelength (nm)"),
	...
	fig2.suptitle(
		r"Imaginary $k_z$" + params_2_print + "\n", fontsize=title_font
	)
	...
	fig1.savefig(
		f"Disp_Diagram_Re_stack{stack_label}",
		bbox_inches="tight",
	)
	fig2.savefig(
		f"Disp_Diagram_Im_stack{stack_label}",
		bbox_inches="tight",
	)
	fig3.savefig(
		f"Disp_Diagram_w(k)_Re_stack{stack_label}",
		bbox_inches="tight",
	)
	fig4.savefig(
...
def E_conc_plot(
	...
	if isinstance(stacks_list[0].layers[which_layer], mode_calcs.Simmo):
		len(stacks_list[0].layers)
		fig1 = plt.figure(
			num=None, figsize=(9, 4), dpi=80, facecolor="w", edgecolor="k"
		)
		...
		for i in range(len(wavelengths)):
			for mode in which_modes:
				E_conc_tmp = np.real(
					stacks_list[i].layers[which_layer].mode_pol[3, mode]
				)
				# E_conc.append(E_conc_tmp)
				ax1.plot(
					wavelengths[i], E_conc_tmp, "bo", linewidth=linesstrength
				)
		if wavelengths[0] != wavelengths[-1]:
		...
		fig1.savefig(
			f"Energy_Concentration_stack{stack_label}",
			bbox_inches="tight",
		)
	else:
		print(
...
def vis_scat_mats(
	scat_mat, nu_prop_PWs=0, wl=None, add_name="", max_scale=None
):
	...
	"""
	fig = plt.figure(
		num=None, figsize=(14, 6), dpi=80, facecolor="w", edgecolor="k"
	)
	if wl is not None:
		add_name = "-wl%(wl)i-%(ti)s" % {"wl": wl, "ti": add_name}
	...
	for i in [1, 2]:
		...
		ax1.plot(
			[-0.5, scat_mat_dim_x - 0.5],
			[half_dim_y, half_dim_y],
			"w",
			linewidth=2,
		)
		ax1.plot(
		...
		ax1.set_xticks(
			[half_dim_y / 2, scat_mat_dim_y - half_dim_y / 2], ["TE", "TM"]
		)
		ax1.set_yticks(
		...
		ax1.plot(
			[-0.5, scat_mat_dim_y - 0.5],
			[half_dim_x + nu_prop_PWs, half_dim_x + nu_prop_PWs],
			"r",
			linewidth=1,
		)
		ax1.plot(
			[half_dim_y + nu_prop_PWs, half_dim_y + nu_prop_PWs],
			[-0.5, scat_mat_dim_x - 0.5],
			"r",
			linewidth=1,
		)
...
def vis_matrix(scat_mat, add_name="", max_scale=None, only_real=False):
	...
	"""
	fig = plt.figure(
		num=None, figsize=(10, 8), dpi=80, facecolor="w", edgecolor="k"
	)
	...
	for i in real_im:
		...
		cbar = fig.colorbar(mat, extend="neither")
		if i == 1:
			cbar.ax.set_ylabel("|Re(matrix)|", fontsize=14)
		if i == 2:
...
def t_func_k_plot_1D(stacks_list, lay_interest=0, pol="TE"):
	...
	ax1.set_ylabel(r"$|E|$")
	plt.savefig(f"k_vector_excitation-lay_{lay_interest}", bbox_inches="tight")
	# fig.clf()
...
def BM_amplitudes(
	...
	if chosen_BMs is None:
		chosen_BMs = range(stacks_list[-1].layers[lay_interest].num_BMs)
	try:
		save_trans = []
		for BM in chosen_BMs:
			...
			if save_npz is True or save_txt is True:
		...
		if save_npz is True:
			np.savez(f"BM_amplitudes-lay_{add_name}", save_trans=save_trans)

		if save_txt is True:
			np.savetxt(
				f"BM_amplitudes-lay_{add_name}.txt",
				save_trans,
				fmt="%18.11f",
			)
		...
		print(
			"BM_amplitudes only works in NanoStruct layers."
	...
	try:
		...
		for pxs in chosen_PWs:
			...
			for stack in stacks_list:
				...
				ix = np.in1d(one_pol_k_space.ravel(), on_axis_kzs).reshape(
					one_pol_k_space.shape
				)
				...
				if vec_index == len(stacks_list[-1].layers) - 1:
					...
					trans = np.abs(
						stack.vec_coef_down[vec_index][axis_indices]
					).reshape(
						-1,
					)
					# Outgoing TM polarisation
					trans += np.abs(
						stack.vec_coef_down[vec_index][
							n_PW_p_pols + axis_indices
						]
					).reshape(
						-1,
					)
		...
		if save_npz is True:
			np.savez(f"PW_amplitudes-lay_{add_name}", save_trans=save_trans)

		if save_txt is True:
			np.savetxt(
				f"PW_amplitudes-lay_{add_name}.txt",
				save_trans,
				fmt="%18.11f",
			)
		...
		print(
			"PW_amplitudes only works in ThinFilm layers."
			"\nPlease select lay_interest !=%i.\n" % lay_interest
		)
	...
	try:
		for stack in stacks_list:
			...
			for pxs in chosen_PWs:
				...
				# Outgoing TM polarisation
				trans += np.abs(
					stack.vec_coef_down[vec_index][n_PW_p_pols + axis_indices]
				)

				if np.abs(this_k_pll2) < np.abs(k**2):
					merit_prop += trans
				if (
					np.abs(k**2)
					< np.abs(this_k_pll2)
					...
					merit_near_ev += trans
				if np.abs(this_k_pll2) > np.abs((n_H * k0) ** 2):
					merit_far_ev += trans
				sum_p_amps += np.abs(pxs) * trans
				sum_amps += (
					np.abs(stack.vec_coef_down[vec_index][axis_indices]) ** 2
					+ np.abs(
						stack.vec_coef_down[vec_index][
							n_PW_p_pols + axis_indices
						]
		...
		ax1 = fig.add_subplot(2, 1, 1)
		if len(store_m_p) != 0:
			ax1.plot(store_x_p, store_m_p, "b", label="prop")
		if len(store_m_ne) != 0:
			ax1.plot(store_x_ne, store_m_ne, "g", label="near ev")
		if len(store_m_fe) != 0:
			ax1.plot(store_x_fe, store_m_fe, "r", label="far ev")
		ax1.plot(
			xvalues,
			store_mean_ev,
			"k",
			label=r"$\Sigma|p| |a_p| / \Sigma|a_p|$",
		)
		...
		if save_pdf is True:
			plt.savefig(
				f"evanescent_merit-lay_{add_name}",
				fontsize=title_font,
				bbox_extra_artists=(lgd,),
				bbox_inches="tight",
			)
		...
		if save_txt is True:
			...
			np.savetxt(
				f"prop_diff_order-lay_{add_name}.txt", av_diff, fmt="%18.11f"
			)
			av_diff = [np.mean(store_m_ne)]
			np.savetxt(
				f"near_ev_diff_order-lay_{add_name}.txt",
				av_diff,
				fmt="%18.11f",
			)
			av_diff = [np.mean(store_m_fe)]
			np.savetxt(
				f"far_ev_diff_order-lay_{add_name}.txt",
				av_diff,
				fmt="%18.11f",
			)
			av_diff = [np.mean(store_mean_ev)]
			np.savetxt(
				f"average_diff_order-lay_{add_name}.txt",
				av_diff,
				fmt="%18.11f",
			)
		...
		print(
			"evanescent_merit only works in ThinFilm layers."
			"\nPlease select lay_interest !=%i.\n" % lay_interest
		)
...
def fields_in_plane(
	...
	for pstack in stacks_list:
		z_values = np.array(z_values) / float(
			pstack.layers[lay_interest].structure.period
		)
		...
		if isinstance(pstack.layers[lay_interest], mode_calcs.Simmo):
			...
			try:
				...
				for z_value in z_values:
					...
					pstack.layers[lay_interest].wl_norm()
					...
					vec_coef = np.concatenate(
						(
							pstack.vec_coef_down[vec_index],
				...
				print(
					"fields_in_plane cannot plot fields in 1D-arrays."
		...
		# plot fields in Plane Wave Basis using python routine.
			...
			for i in range(num_lays):
				if i == 0 or i == num_lays - 1:
					pass
				else:
					heights_list.append(pstack.layers[i].structure.height_nm)
				try:
			...
			if lay_interest == 0:
				name_lay = "0_Substrate"
			elif lay_interest == num_lays - 1:
				name_lay = f"{num_lays - 1}_Superstrate"
			else:
				name_lay = f"Thin_Film_{lay_interest}"
			...
			vec_coef_down = np.array(
				pstack.vec_coef_down[num_lays - 1 - lay_interest]
			).flatten()
			...
			if lay_interest == 0:
				...
				vec_coef_up = np.array(
					pstack.vec_coef_up[num_lays - 1 - lay_interest]
				).flatten()
			...
			E_TE_x_array = np.zeros(
				(np.size(x1), np.size(y1), np.size(z1)), dtype="complex"
			)
			E_TE_y_array = np.zeros(
				(np.size(x1), np.size(y1), np.size(z1)), dtype="complex"
			)
			E_TE_z_array = np.zeros(
				(np.size(x1), np.size(y1), np.size(z1)), dtype="complex"
			)
			E_TM_x_array = np.zeros(
				(np.size(x1), np.size(y1), np.size(z1)), dtype="complex"
			)
			E_TM_y_array = np.zeros(
				(np.size(x1), np.size(y1), np.size(z1)), dtype="complex"
			)
			E_TM_z_array = np.zeros(
				(np.size(x1), np.size(y1), np.size(z1)), dtype="complex"
			)
			...
			for z in range(np.size(z1)):
				for y in range(np.size(y1)):
					for x in range(np.size(x1)):
						if (
							pstack.layers[lay_interest].structure.height_nm
							== "semi_inf"
						):
							expo_down = np.exp(
								1j
						...
						E_TE_x = np.sum(
							eta_TE_x_down * expo_down + eta_TE_x_up * expo_up
						)
						E_TE_y = np.sum(
							eta_TE_y_down * expo_down + eta_TE_y_up * expo_up
						)
						E_TE_z = np.sum(
							eta_TE_z_down * expo_down + eta_TE_z_up * expo_up
						)
						E_TM_x = np.sum(
							eta_TM_x_down * expo_down + eta_TM_x_up * expo_up
						)
						E_TM_y = np.sum(
							eta_TM_y_down * expo_down + eta_TM_y_up * expo_up
						)
						E_TM_z = np.sum(
							eta_TM_z_down * expo_down + eta_TM_z_up * expo_up
						)
			...
			E_tot_array = np.sqrt(
				E_x_array * np.conj(E_x_array)
				+ E_y_array * np.conj(E_y_array)
				+ E_z_array * np.conj(E_z_array)
			)
			...
			for re_im in ["real", "imag"]:
				for z_of_xy in range(np.size(z1)):
					...
					for i in range(len(E_super)):
						...
						plt.contourf(x_axis, y_axis, E_slice, 35, cmap=cmap)
					...
					if save_pdf:
						plt.savefig(
							f"{dir_name}/stack_{stack_num}_{re_im}_E_{name_lay}_slice={z1[z_of_xy]}_wl={wl}.pdf"
						)
...
def fields_interpolator_in_plane(pstack, lay_interest=1, z_value=0.1):
	"""Field linear interpolators in the x-y plane.
	...
	if isinstance(pstack.layers[lay_interest], mode_calcs.Simmo):
		...
		try:
			...
			ReEz = matplotlib.tri.LinearTriInterpolator(
				triang6p, v_Ez6p.real, trifinder=finder
			)
			ImEz = matplotlib.tri.LinearTriInterpolator(
				triang6p, v_Ez6p.imag, trifinder=finder
			)
			AbsE = matplotlib.tri.LinearTriInterpolator(
				triang6p, v_E6p, trifinder=finder
			)
...
def fields_vertically(
	...
	for pstack in stacks_list:
		num_lays = len(pstack.layers)
		pstack.layers[0].structure.period
		pstack.layers[0].max_order_PWs
		...
		if isinstance(layer, mode_calcs.Anallo):
			if layer.structure.world_1d is True:
				...
				slice_types = [
					"xz"
				]  # ,'yz','diag+','diag-'] #rest not implemented
				if gradient is not None:
					slice_types.append(("special+", "special-"))
		else:
		...
		for sli in slice_types:
			...
			for E in E_fields:
				...
				for max_E_field in [0, 1]:
					...
					for lay in range(num_lays):
						...
						if isinstance(layer, mode_calcs.Simmo):
							...
							gmsh_file_pos = (
								f"stack_{stack_num}_lay_{name_lay}_"
							)

							vec_coef_fem = np.concatenate(
								(
									pstack.vec_coef_down[vec_index],
									pstack.vec_coef_up[vec_index],
								)
							...
							if layer.structure.periodicity == "2D_array":
								if max_E_field == 0 and E == "Re(E_x)":
									...
									z_plot = np.linspace(
										h_list[lay], h_list[lay + 1], 2
									)
									(y_axis_plot, x_axis) = np.meshgrid(
										z_plot, [0, 1]
									)
									...
									CS.set_clim(0, 1)
									if abs(ind_h_list[lay]) < 0.05 * np.sum(
										ind_h_list
									):
										tick_half = (
											h_list[lay] + h_list[lay + 1]
										) / 2.0
										ax1.yaxis.set_ticks([tick_half])
										ticktitles = [f"{tick_half:3.2f}"]
										ax1.yaxis.set_ticklabels(ticktitles)
									elif abs(ind_h_list[lay]) < 0.25 * np.sum(
										ind_h_list
									):
										tick_half = (
											h_list[lay] + h_list[lay + 1]
										) / 2.0
										tick_int = ind_h_list[lay] / 4.0
										ticks = [
											tick_half - tick_int,
											tick_half,
											tick_half + tick_int,
							...
							# If 1D_array plot fields as follows.
								...
								if E[-3:] != "(E)":
									...
									for BM in range(layer.num_BMs):
										...
										for h in range(len(z_range)):
											...
											P_down = np.exp(
												...
												* beta
												* (ind_h_list[lay] - hz)
											)  # Introduce Propagation in -z
											P_up = np.exp(
												1j * beta * hz
											)  # Introduce Propagation in +z

											coef_down = (
												vec_coef_fem[BM] * P_down
											)
											coef_up = (
												vec_coef_fem[
													BM + layer.num_BMs
									...
									if max_E_field == 0 and E[0] == "R":
										if E[5] == "x":
											E_fields_tot.append(
												E_slice * np.conj(E_slice)
											)
											epsE_fields_tot.append(
												np.zeros(np.shape(E_slice))
											)
										elif E[5] == "y" or E[5] == "z":
											E_fields_tot[lay] += (
												E_slice * np.conj(E_slice)
									...
									if max_E_field == 0:
										...
										epsE_fields_tot[lay] = np.dot(
											type_el, (E_fields_tot[lay])
										)
									else:
								...
								if E[-3:] != "(E)":  # Re(x,y,z), Im(x,y,z)
									...
									if max_E is None or max_E_lay > max_E:
										max_E = max_E_lay
									min_E_lay = np.min(E_slice)
									if min_E is None or min_E_lay < min_E:
										min_E = min_E_lay
								elif E == "Re(E)":
									max_E_lay = np.max(
										np.real(np.sqrt(E_fields_tot[lay]))
									)
									if max_E is None or max_E_lay > max_E:
										max_E = max_E_lay
									min_E_lay = np.min(
						...
						# ThinFilm layer, using Plane Wave basis.
							...
							if lay == num_lays - 1 and no_incoming is True:
								vec_coef_down = np.zeros(
									(2 * num_pws), dtype="complex"
								)
							else:
								vec_coef_down = np.array(
									pstack.vec_coef_down[vec_index]
							...
							x_axis = np.zeros(
								(nu_pts_hori, nu_pts_hori), dtype="float"
							)
							y_axis = np.zeros(
								(nu_pts_hori, nu_pts_hori), dtype="float"
							)
							...
							E_field = np.zeros(
								(np.size(x1), np.size(y1), np.size(z_range)),
								dtype="complex",
							)
							...
							for z in range(np.size(z_range)):
								for y in range(np.size(y1)):
									for x in range(np.size(x1)):
										...
										E_field[x, y, z] = np.sum(
											eta_TE_down * expo_down
											+ eta_TE_up * expo_up
										) + np.sum(
											eta_TM_down * expo_down
											+ eta_TM_up * expo_up
										)
							...
							if lay == 0:
								...
								z_plot = np.linspace(
									h_list[lay], h_list[lay + 1], nu_pts_vert
								)
							(y_axis_plot, x_axis) = np.meshgrid(
								z_plot, x_range
							)
				...
				cb.mappable.set_clim(min_E, max_E)
				if E == "eps_abs(E)":
	...
	for pstack in stacks_list:
		try:
			...
			if lay_interest == 0:
				...
				name_lay = "%i_Superstrate" % (num_lays - 1)
			else:
				name_lay = "%i_Thin_Film" % lay_interest
			...
			for i in range(len(xyz_values)):
				(x1, y1, z1) = np.array(
					xyz_values[i]
				)  # /float(pstack.layers[lay_interest].structure.period)
				...
				if (
					...
					calc_expo_down = np.exp(
						1j * (alpha * x1 + beta * y1 - gamma * z1)
					)
				else:
					calc_expo_down = np.exp(
						1j
				...
				calc_E_TE_x = np.sum(
					eta_TE_x_down * calc_expo_down + eta_TE_x_up * calc_expo_up
				)
				calc_E_TE_y = np.sum(
					eta_TE_y_down * calc_expo_down + eta_TE_y_up * calc_expo_up
				)
				calc_E_TE_z = np.sum(
					eta_TE_z_down * calc_expo_down + eta_TE_z_up * calc_expo_up
				)
				calc_E_TM_x = np.sum(
					eta_TM_x_down * calc_expo_down + eta_TM_x_up * calc_expo_up
				)
				calc_E_TM_y = np.sum(
					eta_TM_y_down * calc_expo_down + eta_TM_y_up * calc_expo_up
				)
				calc_E_TM_z = np.sum(
					eta_TM_z_down * calc_expo_down + eta_TM_z_up * calc_expo_up
				)
			...
			calc_E_tot_array = np.sqrt(
				calc_E_x_array * np.conj(calc_E_x_array)
				+ calc_E_y_array * np.conj(calc_E_y_array)
				+ calc_E_z_array * np.conj(calc_E_z_array)
			)

			np.savez(
				f"{dir_name}/{stack_num}_E_calc_points_{name_lay}_wl_{wl}",
				calc_E_x_array=calc_E_x_array,
				calc_E_y_array=calc_E_y_array,
				calc_E_z_array=calc_E_z_array,
				calc_E_tot_array=calc_E_tot_array,
			)

			np.savetxt(
				f"{dir_name}/{stack_num}_E_calc_points_{name_lay}_wl_{wl}.txt",
				np.array(
					[
						np.real(calc_E_x_array),
						np.imag(calc_E_x_array),
						np.real(calc_E_y_array),
						np.imag(calc_E_y_array),
						np.real(calc_E_z_array),
						np.imag(calc_E_z_array),
						np.real(calc_E_tot_array),
					]
				),
			)
			stack_num += 1
			...
			print(
				"field_values can only calculate field values in ThinFilms."
				"\nPlease select a different lay_interest.\n"
			)
...
def Bloch_fields_1d(stacks_list, lay_interest=None, save_pdf=False):
	"""
			...
			try:
				...
				for BM in range(meat.num_BMs):
					...
					for i in range(len(fields)):
						...
						BM_sol = meat.sol1[int(i / 2), :, BM, :]
						if E[0] == "R":
							plot_sol = [np.real(BM_sol[0, 0])]
						else:
							plot_sol = [np.imag(BM_sol[0, 0])]
						plot_sol_abs[0] += np.real(
							np.sqrt((BM_sol[0, 0]) * np.conj(BM_sol[0, 0]))
						)
						plot_sol_eps_abs[0] += np.real(
							eps[struct.type_el[0] - 1]
							* np.sqrt((BM_sol[0, 0]) * np.conj(BM_sol[0, 0]))
						)
						for x in range(struct.n_msh_el - 1):
							if E[0] == "R":
								...
								plot_sol.append(
									np.real(BM_sol[2, x] + BM_sol[0, x + 1])
									/ 2
								)
							else:
								plot_sol.append(np.imag(BM_sol[1, x]))
								plot_sol.append(
									np.imag(BM_sol[2, x] + BM_sol[0, x + 1])
									/ 2
								)
							plot_sol_abs[2 * x + 1] += np.real(
								np.sqrt((BM_sol[1, x]) * np.conj(BM_sol[1, x]))
							)
							plot_sol_abs[2 * x + 2] += np.real(
								np.sqrt(
									((BM_sol[2, x] + BM_sol[0, x + 1]) / 2)
									* np.conj(
										(BM_sol[2, x] + BM_sol[0, x + 1]) / 2
									)
								)
							)
							plot_sol_eps_abs[2 * x + 1] += np.real(
								eps[struct.type_el[x] - 1]
								* np.sqrt(
									(BM_sol[1, x]) * np.conj(BM_sol[1, x])
								)
							)
							plot_sol_eps_abs[2 * x + 2] += np.real(
								eps[struct.type_el[x] - 1]
								* np.sqrt(
									((BM_sol[2, x] + BM_sol[0, x + 1]) / 2)
									* np.conj(
						...
						plot_sol_abs[-2] += np.real(
							np.sqrt((BM_sol[1, x]) * np.conj(BM_sol[1, x]))
						)
						plot_sol_abs[-1] += np.real(
							np.sqrt((BM_sol[2, x]) * np.conj(BM_sol[2, x]))
						)
						plot_sol_eps_abs[-2] += np.real(
							eps[struct.type_el[-1] - 1]
							* np.sqrt((BM_sol[1, x]) * np.conj(BM_sol[1, x]))
						)
						plot_sol_eps_abs[-1] += np.real(
							eps[struct.type_el[-1] - 1]
							* np.sqrt((BM_sol[2, x]) * np.conj(BM_sol[2, x]))
						)
						...
						if E == "Re(E_x)":
							plot_sol_2_FTx = plot_sol
						if E == "Im(E_x)":
							plot_sol_2_FT2x = plot_sol
						if E == "Re(E_y)":
							plot_sol_2_FT = plot_sol
						if E == "Im(E_y)":
							plot_sol_2_FT2 = plot_sol
					...
					plt.suptitle(
						r"BM %(BM)i, k$_z$ = %(re)7.3f + %(im)7.3fi (d)"
						% {
							"re": np.real(meat.k_z[BM]),
							"im": np.imag(meat.k_z[BM]),
							"BM": BM,
						}
					)
					if save_pdf:
						plt.savefig(
							dir_name + "/" + name_lay, bbox_inches="tight"
						)
						# fig.clf()
