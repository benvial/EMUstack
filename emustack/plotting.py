    ...
    if isinstance(param_layer, objects.NanoStruct):
        ...
        if param_layer.periodicity == "2D_array":
            ...
            if param_layer.inc_shape == "ellipse":
                params_2_print += "\nEllipticity = %(rad)5.3f " % {
                    "rad": param_layer.ellipticity
                }
...
def t_r_a_plots(
    ...
    if xvalues is None:
        if (
            ...
            xvalues = [
                np.sqrt(
                    s.layers[0].light.k_pll[0] ** 2 + s.layers[0].light.k_pll[1] ** 2
                )
                for s in stacks_list
            ]
    ...
    if ult_eta is True:
        ...
        params_2_print += r"$\eta$ = %(Efficiency)6.2f" % {
    ...
    for i in range(nu_layers):
        ...
        if spectra_name == "Lay_Absorb":
            ...
            if i == nu_layers - 2:
                ax1.set_ylabel("Bottom Layer")
            suptitle_w_params = (
                "Transmittance in each layer" + add_name + "\n" + params_2_print
            )
            plt.suptitle(suptitle_w_params, fontsize=title_font)
        ...
        if save_txt == True:
            if i != nu_layers - 1:
                np.savetxt(
                    ...
                    % {"s": lay_spec_name, "i": i, "bon": stack_label, "add": add_name},
                    av_array,
                    fmt="%18.11f",
                )
            else:
                np.savetxt(
                    "%(s)s_stack%(bon)s%(add)s.txt"
                    % {"s": lay_spec_name, "bon": stack_label, "add": add_name},
                    av_array,
                    fmt="%18.11f",
                )
        if save_pdf == True:
            plt.savefig(
                "%(s)s_stack%(bon)s%(add)s"
...
def EOT_plot(
    stacks_list, wavelengths, pol="TM", params_layer=1, add_name="", savetxt=False
):
    ...
    if savetxt == True:
        np.savetxt(
            "Trans_%(s)s%(add)s.txt" % {"s": plot_name, "add": add_name},
            list(zip(wavelengths, T_00)),
            fmt="%10.6f",
        )
        np.savetxt(
            "Refl_%(s)s%(add)s.txt" % {"s": plot_name, "add": add_name},
...
def J_short_circuit(
    ...
    np.savetxt(
        "J_sc_stack%(bon)s%(add)s.txt" % {"bon": stack_label, "add": add_name},
        np.array([J]),
        fmt="%10.6f",
    )
    return J
...
def ult_efficiency(
    ...
    nums_2_print = params_2_print.split()
    np.savetxt(
        "Efficiency_stack%(bon)s%(add)s.txt" % {"bon": stack_label, "add": add_name},
        np.array([Efficiency]),
        fmt="%8.6f",
    )
    return Efficiency
...
def omega_plot(stacks_list, wavelengths, params_layer=1, stack_label=1):
    ...
    for l in range(num_layers):
        ...
        ax3.set_ylabel(r"Frequency ($\omega$d/2$\pi$c)"), ax4.set_ylabel(
            r"Frequency ($\omega$d/2$\pi$c)"
        )
        if l == 0:
        ...
        if isinstance(stacks_list[0].layers[l], mode_calcs.Anallo):
            ...
            ax3.plot(
                np.real(ns) * normed_omegas, normed_omegas, "k", linewidth=linesstrength
            )
    ...
    fig1.savefig(
        "Disp_Diagram_Re_stack%(bon)s" % {"bon": stack_label}, bbox_inches="tight"
    )
    fig2.savefig(
        "Disp_Diagram_Im_stack%(bon)s" % {"bon": stack_label}, bbox_inches="tight"
    )
    fig3.savefig(
        "Disp_Diagram_w(k)_Re_stack%(bon)s" % {"bon": stack_label}, bbox_inches="tight"
    )
    fig4.savefig(
        "Disp_Diagram_w(k)_Im_stack%(bon)s" % {"bon": stack_label}, bbox_inches="tight"
    )
...
def E_conc_plot(
    stacks_list, which_layer, which_modes, wavelengths, params_layer=1, stack_label=1
):
    ...
    if isinstance(stacks_list[0].layers[which_layer], mode_calcs.Simmo):
        ...
        fig1.suptitle(
            "Energy Concentration = " + r"$E_{cyl} / E_{cell}$" + "\n" + params_2_print,
            fontsize=title_font,
        )
        fig1.savefig(
            "Energy_Concentration_stack%(bon)s" % {"bon": stack_label},
            bbox_inches="tight",
        )
    else:
        print(
...
def vis_scat_mats(scat_mat, nu_prop_PWs=0, wl=None, add_name="", max_scale=None):
    ...
    for i in [1, 2]:
        ...
        ax1.plot(
            [-0.5, scat_mat_dim_x - 0.5], [half_dim_y, half_dim_y], "w", linewidth=2
        )
        ax1.plot(
            [half_dim_x, half_dim_x], [-0.5, scat_mat_dim_y - 0.5], "w", linewidth=2
        )
...
def BM_amplitudes(
    ...
    try:
        ...
        if save_txt == True:
            np.savetxt("BM_amplitudes-lay_%s.txt" % add_name, save_trans, fmt="%18.11f")
...
def PW_amplitudes(
    ...
    try:
        ...
        for pxs in chosen_PWs:
            ...
            for stack in stacks_list:
                ...
                # Superstrate - if not up & down then take only upwards propagating.
                    ...
                    trans = np.abs(stack.vec_coef_up[vec_index][axis_indices]).reshape(
                        -1,
                    )
                    trans += np.abs(
                        stack.vec_coef_up[vec_index][n_PW_p_pols + axis_indices]
                    ).reshape(
                        -1,
                    )
                    if (
        ...
        if save_txt == True:
            np.savetxt("PW_amplitudes-lay_%s.txt" % add_name, save_trans, fmt="%18.11f")
...
def evanescent_merit(
    ...
    try:
        for stack in stacks_list:
            ...
            for pxs in chosen_PWs:
                ...
                sum_amps += (
                    ...
                    + np.abs(stack.vec_coef_down[vec_index][n_PW_p_pols + axis_indices])
                    ** 2
                )
        ...
        handles, labels = ax1.get_legend_handles_labels()
        lgd = ax1.legend(
            handles, labels, ncol=4, loc="upper center", bbox_to_anchor=(0.5, 1.6)
        )
        ...
        if save_pdf == True:
            plt.savefig(
                "evanescent_merit-lay_%s" % add_name,
                fontsize=title_font,
                bbox_extra_artists=(lgd,),
                bbox_inches="tight",
            )
        ...
        if save_txt is True:
            ...
            np.savetxt("prop_diff_order-lay_%s.txt" % add_name, av_diff, fmt="%18.11f")
            av_diff = [np.mean(store_m_ne)]
            np.savetxt(
                "near_ev_diff_order-lay_%s.txt" % add_name, av_diff, fmt="%18.11f"
            )
            av_diff = [np.mean(store_m_fe)]
            np.savetxt(
                "far_ev_diff_order-lay_%s.txt" % add_name, av_diff, fmt="%18.11f"
            )
            av_diff = [np.mean(store_mean_ev)]
            np.savetxt(
                "average_diff_order-lay_%s.txt" % add_name, av_diff, fmt="%18.11f"
            )
...
def fields_in_plane(
    stacks_list, lay_interest=1, z_values=[0.1, 3.6], nu_calc_pts=51, save_pdf=False
):
    """
    ...
    for pstack in stacks_list:
            ...
            try:
                ...
                for z_value in z_values:
                    ...
                    vec_coef = np.concatenate(
                        (pstack.vec_coef_down[vec_index], pstack.vec_coef_up[vec_index])
                    )
        ...
        # plot fields in Plane Wave Basis using python routine.
            ...
            if lay_interest == 0:
                ...
                name_lay = "%(i)s_Superstrate" % {"i": num_lays - 1}
            else:
                name_lay = "Thin_Film_%(i)s" % {"i": lay_interest}
                ...
                for y in range(np.size(y1)):
                    for x in range(np.size(x1)):
                        if (
                            ...
                            z1[z]
                            <= float(pstack.layers[lay_interest].structure.height_nm)
                            / period
                        ):
                            expo_down = np.exp(
                                1j
            ...
            for re_im in ["real", "imag"]:
                for z_of_xy in range(np.size(z1)):
                    fig1 = plt.figure(
                        num=None, figsize=(12, 21), dpi=80, facecolor="w", edgecolor="k"
                    )
                    ...
                    if save_pdf:
                        plt.savefig(
                            "%(dir_name)s/stack_%(stack_num)s_%(p)s_E_%(name)s_slice=%(z_pos)s_wl=%(wl)s.pdf"
                            % {
                                "dir_name": dir_name,
                                "wl": wl,
                                "p": re_im,
            ...
            for E in E_fields:
                ...
                for max_E_field in [0, 1]:
                    ...
                    for lay in range(num_lays):
                        ...
                        nu_pts_vert = int(
                            factor_pts_vert
                            * (np.real(np.max(eps)) * ind_h_list[lay] / wl_normed)
                        )
                        if nu_pts_vert < 11:
                            nu_pts_vert = 11
                        if lay == 0:
                        ...
                        if isinstance(layer, mode_calcs.Simmo):
                            ...
                            if layer.structure.periodicity == "2D_array":
                                if max_E_field == 0 and E == "Re(E_x)":
                                    ...
                                    scale_plot = 2.0
                                    shift_x_plot = -0.5
                                    h_normed = float(layer.structure.height_nm) / float(
                                        layer.structure.period
                                    )
                            ...
                            # If 1D_array plot fields as follows.
                                ...
                                E_slice = np.zeros(
                                    (struct.n_msh_pts, nu_pts_vert), dtype="complex128"
                                )
                                ...
                                if E[-3:] != "(E)":
                                    ...
                                    for BM in range(layer.num_BMs):
                                        ...
                                        for h in range(len(z_range)):
                                            ...
                                            coef_up = (
                                                vec_coef_fem[BM + layer.num_BMs] * P_up
                                            )
                                            if E[5] == "z":
                                                coef_tot = (
                                                    coef_up - coef_down
                                                ) / beta  # Taking into account the change of variable for Ez
                                            else:
                                    ...
                                    if max_E_field == 0 and E[0] == "R":
                                        if E[5] == "x":
                                            ...
                                            E_fields_tot[lay] += E_slice * np.conj(
                                                E_slice
                                            )
                                elif E == "eps_abs(E)":
                                    if max_E_field == 0:
                                ...
                                if E[-3:] != "(E)":  # Re(x,y,z), Im(x,y,z)
                                    ...
                                    if max_E == None or max_E_lay > max_E:
                                        max_E = max_E_lay
                                    min_E_lay = np.min(E_slice)
                                    if min_E == None or min_E_lay < min_E:
                                        min_E = min_E_lay
                                elif E == "Re(E)":
                                    max_E_lay = np.max(
                            ...
                            if sli == "xz":
                                ...
                                if max_E_field == 0:
                                    if E[-3:] != "(E)" and E[0] == "R":
                                        ...
                                        epsE_fields_tot[lay] = np.real(eps) * (
                                            E_fields_tot[lay]
                                        )
                ...
                if save_pdf:
                    plt.savefig(
                        "%(dir_name)s/stack_%(stack_num)s_E_%(slice)s_slice_%(comp)s%(add)s.pdf"
                        % {
                            "dir_name": dir_name,
                            "comp": E,
                            "slice": sli,
...
def field_values(stacks_list, lay_interest=0, xyz_values=[(0.1, 0.1, 0.1)]):
    ...
    for pstack in stacks_list:
        try:
            ...
            np.savez(
                "%(dir_name)s/%(stack_num)s_E_calc_points_%(name)s_wl_%(wl)s"
                % {
                    "dir_name": dir_name,
                    "wl": wl,
                    "name": name_lay,
            ...
            np.savetxt(
                "%(dir_name)s/%(stack_num)s_E_calc_points_%(name)s_wl_%(wl)s.txt"
                % {
                    "dir_name": dir_name,
                    "wl": wl,
                    "name": name_lay,
            ...
            stack_num += 1
            super_points.append(
                [calc_E_x_array, calc_E_y_array, calc_E_z_array, calc_E_tot_array]
            )
...
def fields_3d(stacks_list, lay_interest=1):
    ...
    for pstack in stacks_list:
        try:
            ...
            layer_name = (
                "Lay_" + zeros_int_str(lay_interest) + "Stack_" + str(stack_num)
            )
                    ...
                    for i in range(len(fields)):
                        ...
                        for x in range(struct.n_msh_el - 1):
                            if E[0] == "R":
                                ...
                                plot_sol.append(
                                    np.real(BM_sol[2, x] + BM_sol[0, x + 1]) / 2
                                )
                            else:
                                plot_sol.append(np.imag(BM_sol[1, x]))
                                plot_sol.append(
                                    np.imag(BM_sol[2, x] + BM_sol[0, x + 1]) / 2
                                )
                            ...
                            plot_sol_abs[2 * x + 2] += np.real(
                                np.sqrt(
                                    ...
                                    * np.conj((BM_sol[2, x] + BM_sol[0, x + 1]) / 2)
                                )
                            )
